...
A `string`/`*string` passed via `clickhouse.Named` is treated as the literal value and escaped by the driver, so it round-trips byte-for-byte. Use an actual newline or tab in the Go string when that is what you want on the wire:
