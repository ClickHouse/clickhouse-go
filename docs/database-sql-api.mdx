...
* `hosts` - comma-separated list of single address hosts for load-balancing and failover (bracketed IPv6 supported). Parsed with `lib/churl` and `net.SplitHostPort` - see [Connecting to Multiple Nodes](/integrations/language-clients/go/configuration#connecting-to-multiple-nodes).
