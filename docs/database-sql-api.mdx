...
A `string`/`*string` or `[]byte`/`*[]byte` passed via `clickhouse.Named` is treated as the literal value and escaped by the driver, so it round-trips byte-for-byte. Use an actual newline or tab in the Go value when that is what you want on the wire:

```go
clickhouse.Named("message", "line 1\nline 2") // literal newline — round-trips
```

For already-serialized parameter text in ClickHouse's `Escaped` format, use `WithParameters` instead; see [Server-side query parameters](/integrations/language-clients/go/clickhouse-api#server-side-query-parameters) for the full explanation of types, escaping, and how `{name:Type}` differs from client-side binding.
