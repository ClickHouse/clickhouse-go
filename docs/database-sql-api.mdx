...
* `hosts` - comma-separated list of single address hosts for load-balancing and failover (bracketed IPv6 supported). Parsed with `lib/churl` (`net/url.Parse` fork), then `strings.Split` and `net.SplitHostPort` - see [Connecting to Multiple Nodes](/integrations/language-clients/go/configuration#connecting-to-multiple-nodes).
* `username/password` - auth credentials (DSN userinfo and/or `username`/`password` query params; query overrides userinfo). Credentials are cluster-wide across all hosts - see [Authentication](/integrations/language-clients/go/configuration#authentication)
...
The standard API supports ClickHouse server-side query parameters using typed `{name:Type}` placeholders and `clickhouse.Named` values:
...
Parameter strings use ClickHouse's `Escaped` text format. In particular, use a Go raw literal such as `` `line 1\nline 2` `` or an interpreted literal such as `"line 1\\nline 2"` when ClickHouse should receive a newline. Passing `"line 1\nline 2"` sends an actual newline before ClickHouse parses the parameter and is rejected.

See [Server-side query parameters](/integrations/language-clients/go/clickhouse-api#server-side-query-parameters) for the full explanation of types, escaping, raw versus interpreted Go string literals, and how `{name:Type}` differs from client-side binding.

[Full standard API example](https://github.com/ClickHouse/clickhouse-go/blob/main/examples/std/query_parameters.go)
