...
The driver parses the DSN with `net/url.Parse`. Each peer is then split with `net.SplitHostPort`. If Go 1.26 `url.Parse` rejects or collapses the host list (http(s) with `GODEBUG=urlstrictcolons`, or bracketed IPv6 lists), the DSN is parsed as a single-host URL and the original peers are restored.
