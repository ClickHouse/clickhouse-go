...
The driver parses the DSN with `lib/churl`, a version-stable fork of `net/url` (see [#1787](https://github.com/ClickHouse/clickhouse-go/pull/1787)) that keeps DSN parsing behavior fixed regardless of the Go toolchain version, including comma-separated host lists that the stdlib `net/url` package rejects or collapses as of Go 1.26. Each peer is then split with `net.SplitHostPort`.
