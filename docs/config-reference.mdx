...
Multi-host DSNs use a comma-separated authority (`clickhouse://user:pass@host1:9000,host2:9000/db`). Bracketed IPv6 peers are supported. `Auth` from userinfo / `username` / `password` is cluster-wide (query params override userinfo).
