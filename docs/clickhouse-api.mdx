...
Parameter values are not SQL literals: for a top-level `String` via `Named`, pass the value without SQL quotes — use `hello`, not `'hello'`, because those quote characters would become part of the value. Composite types still use their text syntax, for example `['a', 'b']` for an `Array(String)`, which you would pass through `Parameters`.
