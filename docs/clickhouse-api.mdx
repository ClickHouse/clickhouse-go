...
Query parameter values use ClickHouse's `Escaped` text format. They are not SQL literals and they are not automatically escaped Go strings. For a top-level `String`, pass the value without SQL quotes: use `hello`, not `'hello'`, because those quote characters would become part of the value. Composite types still use their text syntax, for example `['a', 'b']` for an `Array(String)`.

ClickHouse decodes backslash escape sequences while parsing the parameter. Common sequences include `\n` (newline), `\t` (tab), `\r` (carriage return), `\0` (null byte), and `\\` (a literal backslash). `\N` is the whole-value `NULL` marker for nullable types.

Go processes interpreted string literals (`"..."`) before clickhouse-go sees them, while raw string literals (`` `...` ``) preserve backslashes. Write the parameter text as follows:
...
Do not use `"line 1\nline 2"` or `"column 1\tcolumn 2"` when constructing server-side parameter text. Those Go literals already contain an actual newline or tab. In ClickHouse's `Escaped` format, actual newline and tab bytes delimit fields, so the server rejects the parameter because it was not parsed completely. Preserve the backslash for ClickHouse to decode by using either a raw literal or an extra backslash in an interpreted literal.

This distinction applies both to `clickhouse.Parameters` and to string values passed through `clickhouse.Named`. `clickhouse.Parameters` always contains already-serialized parameter text. With `clickhouse.Named`, non-string Go values such as numbers, slices, maps, and times are serialized by the driver, but a string is treated as already-serialized text because it may represent either a `String` or the complete text form of another declared type.
