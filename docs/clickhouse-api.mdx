...
**`clickhouse.Named` (or `sql.Named`) with a `string`/`*string` value** treats the Go string as the literal value. The driver escapes control characters automatically, so the value round-trips byte-for-byte on both protocols. An actual newline or tab in the string is fine:
