...
test:
	cd tests && CLICKHOUSE_VERSION=$(CLICKHOUSE_VERSION) CLICKHOUSE_QUORUM_INSERT=$(CLICKHOUSE_QUORUM_INSERT) go test -race -tags docker -timeout $(CLICKHOUSE_TEST_TIMEOUT) -count=1 -v ./...
