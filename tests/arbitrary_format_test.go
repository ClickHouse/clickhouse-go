...
func createArbitraryFormatTable(t *testing.T, conn driver.Conn, populate bool) string {
	t.Helper()
	ctx := context.Background()
	table := fmt.Sprintf("test_arbitrary_format_%s", RandAsciiString(8))
	require.NoError(t, conn.Exec(ctx, fmt.Sprintf(arbitraryFormatDDL, table)))
	t.Cleanup(func() {
	...
	if populate {
		...
		for _, row := range arbitraryFormatRows {
...
func verifyArbitraryFormatTable(t *testing.T, conn driver.Conn, table string) {
	t.Helper()
	...
	require.Equal(t, len(arbitraryFormatRows), i)
}

// TestArbitraryFormatRoundTrip streams a table out in each format and feeds
...
func TestArbitraryFormatRoundTrip(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol clickhouse.Protocol) {
		for _, format := range []string{"CSV", "JSONEachRow", "Parquet", "ArrowStream"} {
			t.Run(format, func(t *testing.T) {
				...
				source := createArbitraryFormatTable(t, conn, true)
				dest := createArbitraryFormatTable(t, conn, false)

				stream, err := conn.QueryArbitraryFormat(ctx, format,
				...
				require.NoError(t, conn.InsertArbitraryFormat(ctx, format,
					fmt.Sprintf("INSERT INTO %s", dest), bytes.NewReader(payload)))
				verifyArbitraryFormatTable(t, conn, dest)
			})
		}
	})
}

// TestArbitraryFormatCSVContent pins the CSV bytes: server-rendered over HTTP
// and client-rendered over TCP must agree for fidelity-safe types.
func TestArbitraryFormatCSVContent(t *testing.T) {
	...
	TestProtocols(t, func(t *testing.T, protocol clickhouse.Protocol) {
		conn, err := GetNativeConnection(t, protocol, nil, nil, nil)
		require.NoError(t, err)
		table := createArbitraryFormatTable(t, conn, true)

		stream, err := conn.QueryArbitraryFormat(context.Background(), "CSV",
...
// TestArbitraryFormatInsertStripsFormatClause proves the format argument wins
// over a FORMAT clause embedded in the INSERT statement.
func TestArbitraryFormatInsertStripsFormatClause(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol clickhouse.Protocol) {
		conn, err := GetNativeConnection(t, protocol, nil, nil, nil)
		require.NoError(t, err)
		table := createArbitraryFormatTable(t, conn, false)
...
`
		require.NoError(t, conn.InsertArbitraryFormat(context.Background(), "JSONEachRow",
			fmt.Sprintf("INSERT INTO %s FORMAT CSV", table), strings.NewReader(payload)))
		verifyArbitraryFormatTable(t, conn, table)
	})
}

// TestArbitraryFormatUnregisteredTCP verifies the actionable error for a
// format with no client-side codec over the native protocol.
func TestArbitraryFormatUnregisteredTCP(t *testing.T) {
	conn, err := GetNativeConnection(t, clickhouse.Native, nil, nil, nil)
	require.NoError(t, err)

	_, err = conn.QueryArbitraryFormat(context.Background(), "ORC", "SELECT 1")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "Options.FormatCodecs")
	assert.Contains(t, err.Error(), "HTTP")

	err = conn.InsertArbitraryFormat(context.Background(), "ORC", "INSERT INTO t", strings.NewReader(""))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "Options.FormatCodecs")
...
// TestArbitraryFormatMultiBlockInsertTCP pushes more rows than one native
// block holds, forcing the decoder loop to emit multiple blocks.
func TestArbitraryFormatMultiBlockInsertTCP(t *testing.T) {
	conn, err := GetNativeConnection(t, clickhouse.Native, nil, nil, nil)
	require.NoError(t, err)
	ctx := context.Background()
	table := fmt.Sprintf("test_arbitrary_format_%s", RandAsciiString(8))
	...
	require.NoError(t, conn.InsertArbitraryFormat(ctx, "CSV",
...
// TestArbitraryFormatMalformedInsertTCP verifies a row-numbered decode error
// and that the connection pool stays usable after the aborted insert.
func TestArbitraryFormatMalformedInsertTCP(t *testing.T) {
	conn, err := GetNativeConnection(t, clickhouse.Native, nil, nil, nil)
	require.NoError(t, err)
	ctx := context.Background()
	table := fmt.Sprintf("test_arbitrary_format_%s", RandAsciiString(8))
	...
	err = conn.InsertArbitraryFormat(ctx, "CSV",
...
// TestArbitraryFormatMidStreamExceptionHTTP forces a server exception after
// streaming has begun; the reader must surface it as an error after any
// partial data.
func TestArbitraryFormatMidStreamExceptionHTTP(t *testing.T) {
	...
	stream, err := conn.QueryArbitraryFormat(ctx, "CSV",
...
// TestArbitraryFormatCancellationTCP cancels the context mid-read; the reader
// must unblock with an error and the pool must recover.
func TestArbitraryFormatCancellationTCP(t *testing.T) {
	conn, err := GetNativeConnection(t, clickhouse.Native, nil, nil, nil)
	require.NoError(t, err)

	ctx, cancel := context.WithCancel(context.Background())
	stream, err := conn.QueryArbitraryFormat(ctx, "CSV", "SELECT number, randomString(100) FROM numbers(100000000)")
	require.NoError(t, err)
	defer stream.Close()
...
// TestArbitraryFormatCompressionHTTP exercises the gzip transport path in
// both directions.
func TestArbitraryFormatCompressionHTTP(t *testing.T) {
	...
	require.NoError(t, conn.InsertArbitraryFormat(ctx, "JSONEachRow",
		fmt.Sprintf("INSERT INTO %s", dest), bytes.NewReader(payload)))
	verifyArbitraryFormatTable(t, conn, dest)
}

// TestArbitraryFormatCrossProtocolParquet proves client-encoded Parquet is
// accepted by the server: bytes produced by the TCP codec are uploaded over
// HTTP, where the server parses them.
func TestArbitraryFormatCrossProtocolParquet(t *testing.T) {
	...
	source := createArbitraryFormatTable(t, tcpConn, true)
	dest := createArbitraryFormatTable(t, tcpConn, false)

	stream, err := tcpConn.QueryArbitraryFormat(ctx, "Parquet",
		fmt.Sprintf("SELECT id, name, score, ok, created_at, comment FROM %s ORDER BY id", source))
	require.NoError(t, err)
	payload, err := io.ReadAll(stream)
	require.NoError(t, err)
	...
	require.NoError(t, httpConn.InsertArbitraryFormat(ctx, "Parquet",
		fmt.Sprintf("INSERT INTO %s", dest), bytes.NewReader(payload)))
	verifyArbitraryFormatTable(t, tcpConn, dest)
}
