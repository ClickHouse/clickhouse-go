...
func TestColumnarNullTimes(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol clickhouse.Protocol) {
		...
		assertNullTime(t, datetime.Truncate(time.Second), actual[0].DateTimeValue)
		assertNullTime(t, datetimeAlternative.Truncate(time.Second), actual[0].DateTimePointer)
		...
		assertNullTime(t, datetimeAlternative.Truncate(time.Second), actual[2].DateTimeValue)
