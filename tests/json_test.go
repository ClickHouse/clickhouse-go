...
func TestJSONPaths(t *testing.T) {
	...
	for path, expectedValue := range expectedValuesByPath {
		...
		case clickhouse.Dynamic:
			expectedValue = expectedValue.(clickhouse.Dynamic).Any()
		case clickhouse.DynamicWithType:
			expectedValue = expectedValue.(clickhouse.DynamicWithType).Any()
		}

		switch actualValue.(type) {
		case clickhouse.Dynamic:
			actualValue = actualValue.(clickhouse.Dynamic).Any()
		case clickhouse.DynamicWithType:
			actualValue = actualValue.(clickhouse.DynamicWithType).Any()
		}
...
func TestJSON_BatchFlush(t *testing.T) {
	...
	for rows.Next() {
		...
		if i%2 == 0 {
			...
			_, ok = valA.(clickhouse.DynamicWithType)
			require.Equal(t, true, ok)

			require.Equal(t, int64(i), valA.(clickhouse.DynamicWithType).Any())
			require.Equal(t, "Int64", valA.(clickhouse.DynamicWithType).Type())

			valB, ok := row.ValueAtPath("b")
			require.Equal(t, true, ok)
			_, ok = valB.(clickhouse.DynamicWithType)
			require.Equal(t, true, ok)

			require.Equal(t, i%5 == 0, valB.(clickhouse.DynamicWithType).Any())
			require.Equal(t, "Bool", valB.(clickhouse.DynamicWithType).Type())
		} else {
			valC, ok := row.ValueAtPath("c")
			require.Equal(t, true, ok)
			_, ok = valC.(clickhouse.DynamicWithType)
			require.Equal(t, true, ok)

			require.Equal(t, int64(-i), valC.(clickhouse.DynamicWithType).Any())
			require.Equal(t, "Int64", valC.(clickhouse.DynamicWithType).Type())

			valD, ok := row.ValueAtPath("d")
			require.Equal(t, true, ok)
			_, ok = valD.(clickhouse.DynamicWithType)
			require.Equal(t, true, ok)

			require.Equal(t, i%5 != 0, valD.(clickhouse.DynamicWithType).Any())
			require.Equal(t, "Bool", valD.(clickhouse.DynamicWithType).Type())
		}
