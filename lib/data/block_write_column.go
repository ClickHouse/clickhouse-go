...
func (block *Block) WriteUInt8Nullable(c int, v *uint8) error {
	block.buffers[c].Column.Write([]byte{1})
	return block.buffers[c].Column.UInt8(0)
	//block.buffers[c].Column.Write(blo{reflect.ValueOf(uint8(0))})
	//return block.buffers[c].Column.UInt8Nullable(block.Columns[0].def)
