...
func init() {
	...
	for _, typ := range types {
		plainArrayGoTypes = append(plainArrayGoTypes,
			fmt.Sprintf("[]%s", typ.GoType),
			fmt.Sprintf("[]*%s", typ.GoType),
		)
	}
...
func main() {
	for name, tpl := range map[string]struct {
		...
		"array_gen":  {template.Must(template.New("array").Parse(arraySrc)), plainArrayGoTypes},
	} {
